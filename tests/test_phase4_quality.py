...
try:
    ...
    print(
        f"  - ResponseValidator: score={result.score:.2f}, issues={len(result.issues)}", flush=True
    )
...
try:
    from ai_brain.inference.templates import ResponseFormatter, format_response, DisclaimerGenerator

    formatter = ResponseFormatter()
    print("  - ResponseFormatter: initialized", flush=True)

    # Test template formatting
    result = format_response(
        "You should create a budget and track your spending.", mode="chat", confidence=0.75
    )
    print(f"  - format_response: sections = {list(result.sections.keys())}", flush=True)

    # Test disclaimer generation
    disclaimer = DisclaimerGenerator.get_disclaimer(topics=["investment", "tax"], confidence=0.6)
...
try:
    ...
    print(
        f"  - FinancialRulesEngine: valid = {result['is_valid']}, disclaimers = {len(result['required_disclaimers'])}",
        flush=True,
    )

    # Test AIValidationService
    service = AIValidationService()
    print("  - AIValidationService: initialized", flush=True)
...
try:
    from ai_brain.inference.confidence import ConfidenceLevel, ConfidenceCalculator, TORCH_AVAILABLE
    ...
    print(
        f"  - calculate_from_log_probs: score={result.score:.3f}, level={result.level.value}",
        flush=True,
    )
