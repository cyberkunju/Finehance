...
class TestBudgetRoutes:
    ...
    async def test_create_budget_invalid_period(
        ...
        response = await async_client.post(
            ...
            json={
                ...
                "allocations": {"Groceries": "500.00"},
            },
        )

        assert response.status_code == 422  # Validation error

    async def test_create_budget_negative_allocation(
        self, async_client: AsyncClient, test_user, test_db
    ):
    ...
    async def test_get_budget_progress_with_alerts(
        self, async_client: AsyncClient, test_user, test_db
    ):
    ...
    async def test_update_budget_success(self, async_client: AsyncClient, test_user, test_db):
        ...
        response = await async_client.put(
            ...
            json={
                "name": "Updated Name",
                "allocations": {"Groceries": "600.00", "Dining": "200.00"},
            },
        )
    ...
    async def test_update_budget_partial(self, async_client: AsyncClient, test_user, test_db):
        ...
        # Update only name
        response = await async_client.put(
            f"/api/budgets/{budget.id}?user_id={user_id}", json={"name": "Updated Name"}
        )
    ...
    async def test_update_budget_not_found(self, async_client: AsyncClient, test_user, test_db):
        ...
        response = await async_client.put(
            f"/api/budgets/{fake_id}?user_id={user_id}", json={"name": "Updated"}
        )
    ...
    async def test_update_budget_no_updates(self, async_client: AsyncClient, test_user, test_db):
        ...
        response = await async_client.put(f"/api/budgets/{budget.id}?user_id={user_id}", json={})
    ...
    async def test_apply_optimization_without_approval(
        self, async_client: AsyncClient, test_user, test_db
    ):
        ...
        response = await async_client.put(
            ...
            json={"suggestions": [], "user_approved": False},
        )
