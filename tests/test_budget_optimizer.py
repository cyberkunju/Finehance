...
class TestBudgetOptimizer:
    ...
    async def test_analyze_spending_patterns(
        self,
        budget_optimizer,
        test_user,
        db_session
        ...
        # Analyze patterns
        patterns = await budget_optimizer.analyze_spending_patterns(
            test_user.id,
            months=3
        )
    ...
    async def test_suggest_optimizations_overspending(
        ...
        groceries_suggestion = next(
            (s for s in suggestions if s.category == "Groceries"),
            None
        )
    ...
    async def test_suggest_optimizations_underspending(
        ...
        entertainment_suggestion = next(
            (s for s in suggestions if s.category == "Entertainment"),
            None
        )
        ...
        if entertainment_suggestion:
            ...
            assert entertainment_suggestion.suggested_allocation <= entertainment_suggestion.current_allocation or \
                   abs(entertainment_suggestion.suggested_allocation - entertainment_suggestion.current_allocation) < Decimal("1")
    ...
    async def test_balance_suggestions(
        self,
        budget_optimizer,
        sample_budget
    ):
    ...
    async def test_multiple_categories_optimization(
        ...
        assert "Groceries" in categories_with_suggestions or "Entertainment" in categories_with_suggestions
