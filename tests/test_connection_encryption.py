...
class TestConnectionEncryption:
    ...
    async def test_access_token_decryption_on_read(self, db_session: AsyncSession, test_user: User):
        ...
        result = await db_session.execute(select(Connection).filter_by(id=connection_id))
    ...
    async def test_multiple_connections_different_encryption(
    ...
    async def test_connection_with_special_characters_in_token(
        self, db_session: AsyncSession, test_user: User
    ):
