...
class TestAdviceRoutes:
    ...
    async def test_get_personalized_advice_max_recommendations(
        ...
        response = await async_client.get(
            f"/api/advice?user_id={user_id}&max_recommendations=5"
        )
    ...
    async def test_get_savings_opportunities_no_data(
        ...
        response = await async_client.get(
            f"/api/advice/savings-opportunities?user_id={user_id}"
        )
    ...
    async def test_advice_validation_max_recommendations(
        ...
        # Test too high
        response = await async_client.get(
            f"/api/advice?user_id={user_id}&max_recommendations=20"
        )
        assert response.status_code == 422  # Validation error
        
        # Test too low
        response = await async_client.get(
            f"/api/advice?user_id={user_id}&max_recommendations=0"
        )
        assert response.status_code == 422  # Validation error
    ...
    async def test_advice_priority_ordering(
        self, async_client: AsyncClient, test_user, test_db
    ):
