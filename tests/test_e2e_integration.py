...
class TestTransactionManagementFlow:
    ...
    async def test_transaction_crud_flow(
        ...
        response = await client.post(
            "/api/transactions", json=transaction_data, headers=auth_headers
        )
        assert response.status_code == 201
        ...
        update_data = {"amount": 55.00, "description": "Updated: Grocery shopping"}
        response = await client.put(
            f"/api/transactions/{transaction_id}", json=update_data, headers=auth_headers
        )
        assert response.status_code == 200
...
class TestBudgetManagementFlow:
    ...
    @pytest.mark.asyncio
    async def test_budget_lifecycle(self, client: AsyncClient, test_user: User, auth_headers: dict):
        ...
        for txn_data in transactions:
            response = await client.post("/api/transactions", json=txn_data, headers=auth_headers)
            assert response.status_code == 201
        ...
        budget_data = {
            ...
            "allocations": {"Groceries": 400.00, "Dining": 200.00, "Transportation": 150.00},
        }
        response = await client.post("/api/budgets", json=budget_data, headers=auth_headers)
        assert response.status_code == 201
        budget = response.json()
        budget_id = budget["id"]

        # 3. Check budget progress
        response = await client.get(f"/api/budgets/{budget_id}/progress", headers=auth_headers)
        assert response.status_code == 200
        ...
        response = await client.post(f"/api/budgets/{budget_id}/optimize", headers=auth_headers)
        assert response.status_code == 200
...
class TestGoalTrackingFlow:
    ...
    @pytest.mark.asyncio
    async def test_goal_lifecycle(self, client: AsyncClient, test_user: User, auth_headers: dict):
        ...
        response = await client.post("/api/goals", json=goal_data, headers=auth_headers)
        assert response.status_code == 201
        goal = response.json()
        goal_id = goal["id"]
        ...
        response = await client.put(
            f"/api/goals/{goal_id}/progress", json={"amount": 500.00}, headers=auth_headers
        )
        assert response.status_code == 200
        ...
        response = await client.get(f"/api/goals/{goal_id}/risk-alerts", headers=auth_headers)
        assert response.status_code == 200
...
class TestAdviceGenerationFlow:
    ...
    async def test_advice_generation(
        ...
        for txn_data in transactions:
            await client.post("/api/transactions", json=txn_data, headers=auth_headers)

        # 2. Get personalized advice
        response = await client.get("/api/advice", headers=auth_headers)
        assert response.status_code == 200
        ...
        response = await client.get("/api/advice/spending-alerts", headers=auth_headers)
        assert response.status_code == 200

        # 4. Get savings opportunities
        response = await client.get("/api/advice/savings-opportunities", headers=auth_headers)
        assert response.status_code == 200
...
class TestReportingFlow:
    ...
    async def test_report_generation_and_export(
        ...
        response = await client.post(
            "/api/reports/generate", json=report_data, headers=auth_headers
        )
        assert response.status_code == 200
...
class TestFileImportFlow:
    ...
    async def test_file_import_workflow(
        ...
        # 1. Download template
        response = await client.get(
            "/api/import/template", params={"format": "csv"}, headers=auth_headers
        )
        assert response.status_code == 200
...
2024-01-17,Gas Station,40.00,expense,Transportation"""
        ...
        response = await client.post("/api/import/transactions", files=files, headers=auth_headers)
        assert response.status_code == 200
...
class TestPredictionFlow:
    ...
    async def test_expense_forecasting(
        ...
        response = await client.get("/api/predictions", params={"periods": 4}, headers=auth_headers)
...
class TestCompleteUserJourney:
    ...
    async def test_complete_user_journey(self, client: AsyncClient):
        ...
        budget_data = {
            ...
            "period_start": datetime.now().replace(day=1).isoformat(),
            "period_end": (datetime.now().replace(day=1) + timedelta(days=31)).isoformat(),
            "allocations": {"Housing": 1200.00, "Groceries": 400.00, "Utilities": 200.00},
        }
