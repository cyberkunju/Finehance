...
class TestBudgetProgress:
    ...
    async def test_get_budget_progress_no_spending(self, budget_service, test_user, sample_budget):
...
class TestBudgetAlerts:
    ...
    async def test_check_budget_alerts_no_alerts(self, budget_service, test_user, sample_budget):
...
class TestUpdateBudget:
    ...
    async def test_update_budget_name(self, budget_service, test_user, sample_budget):
        ...
        updated = await budget_service.update_budget(
            budget_id=sample_budget.id, user_id=test_user.id, name="Updated Budget Name"
        )
    ...
    async def test_update_budget_allocations(self, budget_service, test_user, sample_budget):
        ...
        new_allocations = {"Groceries": Decimal("600.00"), "Dining": Decimal("400.00")}

        updated = await budget_service.update_budget(
            budget_id=sample_budget.id, user_id=test_user.id, allocations=new_allocations
        )
    ...
    async def test_update_nonexistent_budget(self, budget_service, test_user):
        ...
        updated = await budget_service.update_budget(
            budget_id=uuid4(), user_id=test_user.id, name="New Name"
        )
