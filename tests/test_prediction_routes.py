...
class TestPredictionRoutes:
    ...
    async def test_get_expense_forecasts_success(
        ...
        response = await async_client.get(
            f"/api/predictions/forecast?user_id={user_id}&periods=7"
        )
    ...
    async def test_get_spending_anomalies_no_data(
        ...
        response = await async_client.get(
            f"/api/predictions/anomalies/Groceries?user_id={user_id}"
        )
    ...
    async def test_forecast_validation_periods(
        self, async_client: AsyncClient, test_user, test_db
    ):
        ...
        response = await async_client.get(
            f"/api/predictions/forecast?user_id={user_id}&periods=0"
        )
        assert response.status_code == 422  # Validation error
