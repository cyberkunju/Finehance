...
async def create_test_transaction(service, user_id, **kwargs):
    ...
    return await service.create_transaction(user_id=user_id, transaction_data=transaction_data)
...
class TestTransactionRoutes:
    ...
    async def test_create_transaction_missing_fields(
        ...
        response = await async_client.post(
            f"/api/transactions?user_id={user_id}",
            json={"amount": "50.00", "description": "Missing date and type"},
        )
    ...
    async def test_list_transactions_with_data(
        self, async_client: AsyncClient, test_user, test_db, transaction_service
    ):
    ...
    async def test_update_transaction_partial(
        ...
        response = await async_client.put(
            f"/api/transactions/{transaction.id}?user_id={user_id}", json={"description": "Updated"}
        )
    ...
    async def test_update_transaction_not_found(
        ...
        response = await async_client.put(
            f"/api/transactions/{fake_id}?user_id={user_id}", json={"description": "Updated"}
        )

        assert response.status_code == 404

    async def test_update_transaction_no_updates(
        self, async_client: AsyncClient, test_user, test_db, transaction_service
    ):
        ...
        response = await async_client.put(
            f"/api/transactions/{transaction.id}?user_id={user_id}", json={}
        )
