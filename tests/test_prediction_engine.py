...
class TestTimeSeriesPreparation:
    ...
    async def test_prepare_time_series_with_sufficient_data(
        ...
        series = await prediction_engine.prepare_time_series(
            user_id=test_user.id, category="Groceries", lookback_days=90
        )
    ...
    async def test_prepare_time_series_insufficient_data(
        ...
        series = await prediction_engine.prepare_time_series(
            user_id=test_user.id, category="TestCategory", lookback_days=90
        )
    ...
    async def test_prepare_time_series_fills_missing_dates(
        ...
        series = await prediction_engine.prepare_time_series(
            user_id=test_user.id, category="TestCategory", lookback_days=60
        )
...
class TestARIMAParameters:
    ...
    async def test_select_arima_parameters(self, prediction_engine, test_user, sample_transactions):
...
class TestForecasting:
    ...
    async def test_forecast_expenses_success(
        ...
        result = await prediction_engine.forecast_expenses(
            user_id=test_user.id, category="Groceries", periods=30, lookback_days=90
        )
    ...
    async def test_forecast_expenses_insufficient_data(
        ...
        result = await prediction_engine.forecast_expenses(
            user_id=test_user.id, category="TestCategory", periods=30
        )

        assert result is None

    async def test_forecast_all_categories(self, prediction_engine, test_user, sample_transactions):
        """Test forecasting all categories."""
        forecasts = await prediction_engine.forecast_all_categories(
            user_id=test_user.id, periods=30, lookback_days=90
        )
...
class TestAnomalyDetection:
    ...
    async def test_detect_anomalies(self, prediction_engine, test_user, db_session):
        ...
        anomalies = await prediction_engine.detect_anomalies(
            user_id=test_user.id, category="TestCategory", lookback_days=90, threshold_percent=50.0
        )
...
class TestRecalibration:
    ...
    async def test_recalibrate_model(self, prediction_engine, test_user, sample_transactions):
        """Test model recalibration."""
        result = await prediction_engine.recalibrate_model(
            user_id=test_user.id, category="Groceries", periods=30
        )
