...
from app.ml.categorization_engine import CategorizationEngine, CategoryPrediction
...
def categorization_engine() -> CategorizationEngine:
    ...
    # Use absolute path for model directory to avoid issues
    engine.model_dir = os.path.abspath(engine.model_dir)

    # Ensure global model exists for tests
    global_model_path = os.path.join(engine.model_dir, "global_categorization_model.pkl")
    global_metrics_path = os.path.join(engine.model_dir, "global_categorization_metrics.pkl")
    ...
    if True:
        ...
        descriptions = [preprocess_transaction(desc) for desc, _ in corrections]
        ...
        # Ensure dir exists
        os.makedirs(engine.model_dir, exist_ok=True)
        ...
        joblib.dump(metrics, global_metrics_path)

        # Reload
        engine._load_global_model()
...
class TestCategorizationEngine:
    ...
    @pytest.mark.asyncio
    async def test_categorize_with_global_model(
        self,
    ...
    @pytest.mark.asyncio
    async def test_categorize_dining(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_transportation(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_entertainment(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_salary(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_with_amount(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_categorize_case_insensitive(
        self,
        categorization_engine: CategorizationEngine,
    ):
        """Test that categorization is case-insensitive."""
        result1 = await categorization_engine.categorize("WHOLE FOODS MARKET")
        result2 = await categorization_engine.categorize("whole foods market")
        result3 = await categorization_engine.categorize("Whole Foods Market")
        
        assert result1.category == result2.category == result3.category
        assert result1.category == "Groceries"

    @pytest.mark.asyncio
    async def test_categorize_with_extra_text(
        self,
        categorization_engine: CategorizationEngine,
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_use_global_model_no_user_id(
        self,
        categorization_engine: CategorizationEngine,
    ):
        """Test that global model is used when no user_id provided."""
        assert await categorization_engine.should_use_global_model(None) is True

    @pytest.mark.asyncio
    async def test_should_use_global_model_new_user(
        self,
        categorization_engine: CategorizationEngine,
    ):
        """Test that global model is used for new users."""
        # New user with no personalized model
        assert await categorization_engine.should_use_global_model("new_user_123") is True

    @pytest.mark.asyncio
    async def test_get_model_accuracy_global(
        self,
        categorization_engine: CategorizationEngine,
    ):
        """Test getting global model accuracy."""
        accuracy = await categorization_engine.get_model_accuracy("new_user_123")
    ...
    @pytest.mark.asyncio
    async def test_categorize_multiple_transactions(
        self,
        categorization_engine: CategorizationEngine,
    ):
        ...
        for description, expected_category in test_cases:
            result = await categorization_engine.categorize(description)
    ...
    async def test_confidence_score_range(
        ...
        for desc in descriptions:
            result = await categorization_engine.categorize(desc)
            assert 0.0 <= result.confidence <= 1.0, \
                f"Confidence {result.confidence} out of range for '{desc}'"

    @pytest.mark.asyncio
    async def test_categorize_ambiguous_description(
        self,
        categorization_engine: CategorizationEngine,
    ):
        """Test categorization with ambiguous description."""
        # "Target" could be groceries or shopping
        result = await categorization_engine.categorize("Target")
    ...
    @pytest.mark.asyncio
    async def test_learn_from_correction_trains_model_after_threshold(self, categorization_engine):
        ...
        assert not await categorization_engine.should_use_global_model(user_id)
    
    @pytest.mark.asyncio
    async def test_learn_from_correction_uses_user_model(self, categorization_engine):
    ...
    @pytest.mark.asyncio
    async def test_user_model_accuracy_tracking(self, categorization_engine):
        ...
        # Get accuracy
        accuracy = await categorization_engine.get_model_accuracy(user_id)
