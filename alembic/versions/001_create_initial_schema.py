...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("created_at", sa.DateTime(), nullable=False),
        sa.CheckConstraint(
            "status IN ('ACTIVE', 'EXPIRED', 'ERROR')", name="check_connection_status"
        ),
    ...
    op.create_index(
        "idx_connections_user_status", "connections", ["user_id", "status"], unique=False
    )
    op.create_index(
        "idx_connections_user_institution",
        "connections",
    ...
    op.create_index(
        "idx_transactions_user_category", "transactions", ["user_id", "category"], unique=False
    )
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(), nullable=False),
        sa.CheckConstraint(
            "status IN ('ACTIVE', 'ACHIEVED', 'ARCHIVED')", name="check_goal_status"
        ),
    ...
    op.create_index(
        "idx_financial_goals_user_status", "financial_goals", ["user_id", "status"], unique=False
    )
    op.create_index(
        "idx_financial_goals_user_deadline",
        "financial_goals",
        ["user_id", "deadline"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.CheckConstraint(
            "model_type IN ('CATEGORIZATION', 'PREDICTION')", name="check_model_type"
        ),
        sa.CheckConstraint(
    ...
    op.create_index(
        "idx_ml_models_type_active", "ml_models", ["model_type", "is_active"], unique=False
    )
    op.create_index("idx_ml_models_user_type", "ml_models", ["user_id", "model_type"], unique=False)
    op.create_index(
        "idx_ml_models_user_type_active",
        "ml_models",
        ["user_id", "model_type", "is_active"],
        unique=False,
    )
