...
class HallucinationDetector:
    ...
    def __init__(self):
        ...
        self.specificity_patterns = [
            re.compile(p, re.IGNORECASE) for p in self.SUSPICIOUS_SPECIFICITY
        ]
...
class FinancialFactChecker:
    ...
    IMPOSSIBLE_CLAIMS = [
        ...
        (
            r"(?:double|triple)\s+your\s+money\s+(?:overnight|instantly|immediately)",
    ...
    def __init__(self):
        ...
        self.impossible_patterns = [
            (re.compile(p, re.IGNORECASE), name) for p, name in self.IMPOSSIBLE_CLAIMS
        ]
    ...
    def _check_percentage_bounds(self, response: str) -> List[ValidationIssue]:
        ...
        return_matches = re.findall(
            r"(?:return|earn|make|gain)\s+(?:of\s+)?(\d+(?:\.\d+)?)\s*%", response, re.IGNORECASE
        )
...
class ResponseValidator:
    ...
    def validate(
        ...
        hallucination_issues = self.hallucination_detector.detect(response, context, mode)
        ...
        total_penalty = sum(severity_penalties.get(issue.severity, 0.1) for issue in all_issues)
