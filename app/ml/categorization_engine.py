...
class CategorizationEngine:
    ...
    def __init__(self, model_dir: str = "models"):
        ...
        self.user_corrections: dict[str, List[Tuple[str, str]]] = {}  # user_id -> [(description, category)]
    ...
    def _load_global_model(self) -> None:
        ...
        if not os.path.exists(model_path):
            logger.warning(
                "Global categorization model not found",
        ...
        try:
            ...
            logger.error(
                "Failed to load global categorization model",
                error=str(e)
            )
    ...
    def _load_user_model(self, user_id: str) -> Optional[Pipeline]:
        ...
        try:
            ...
            logger.error(
                "Failed to load user-specific categorization model",
                user_id=user_id,
                error=str(e)
            )
            return None
    ...
    def learn_from_correction(
        ...
        if len(corrections) >= self.min_corrections_for_training:
            ...
            if success:
                logger.info(
                    "User-specific model trained successfully",
                    user_id=user_id
                )
                return True
            else:
                logger.warning(
                    "Failed to train user-specific model",
                    user_id=user_id
                )
                return False
    ...
    def _load_user_corrections(self, user_id: str) -> List[Tuple[str, str]]:
        ...
        try:
            ...
            logger.error(
                "Failed to load user corrections",
                user_id=user_id,
                error=str(e)
            )
            return []
    ...
    def _save_user_corrections(self, user_id: str, corrections: List[Tuple[str, str]]) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to save user corrections",
                user_id=user_id,
                error=str(e)
            )
