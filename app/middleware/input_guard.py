...
class InputGuard:
    ...
    INSTRUCTION_OVERRIDE_PATTERNS = [
        (r"ignore\s+(all\s+)?(previous|prior|above|earlier)\s+(instructions?|rules?|guidelines?|prompts?)", 
         "instruction_override", ThreatLevel.CRITICAL, 20),
        (r"forget\s+(everything|all|what)\s+(you|i)\s+(told|said|learned)", 
         "memory_erasure", ThreatLevel.CRITICAL, 20),
        (r"forget\s+(all\s+)?(previous|prior|earlier)\s+(instructions?|rules?|prompts?)", 
         "memory_erasure_v2", ThreatLevel.CRITICAL, 20),
        (r"disregard\s+(all\s+)?(your|the|my)\s+(instructions?|training|rules?)", 
         "disregard_command", ThreatLevel.CRITICAL, 20),
        (r"override\s+(your|the|system)\s+(instructions?|rules?|settings?)", 
         "override_attempt", ThreatLevel.CRITICAL, 20),
        (r"new\s+instructions?\s*[:\-]", 
         "new_instructions", ThreatLevel.HIGH, 15),
        (r"from\s+now\s+on[,\s]+(you\s+)?(are|will|must|should)", 
         "behavior_change", ThreatLevel.HIGH, 15),
        (r"stop\s+being\s+(a\s+)?(helpful|assistant|ai|safe)", 
         "role_rejection", ThreatLevel.HIGH, 15),
    ]
    
    # Role/persona manipulation - attempts to change AI identity
    ROLE_MANIPULATION_PATTERNS = [
        (r"you\s+are\s+(now|actually|really)\s+(a|an|the)", 
         "role_assignment", ThreatLevel.HIGH, 15),
        (r"you\s+are\s+now\s+(DAN|jailbroken|unrestricted|unlimited)", 
         "role_assignment_jailbreak", ThreatLevel.CRITICAL, 25),
        (r"pretend\s+(to\s+be|you\s+are|that\s+you)", 
         "pretend_command", ThreatLevel.MEDIUM, 10),
        (r"pretend\s+you\s+have\s+no\s+(restrictions?|limits?|rules?|boundaries)", 
         "pretend_unrestricted", ThreatLevel.CRITICAL, 25),
        (r"act\s+(as\s+if|like)\s+you\s+(are|were|have)", 
         "act_as_command", ThreatLevel.MEDIUM, 10),
        (r"roleplay\s+(as|that|where)\s+you", 
         "roleplay_command", ThreatLevel.MEDIUM, 10),
        (r"imagine\s+you\s+(are|were|have)\s+(no|unlimited|free)", 
         "imagination_exploit", ThreatLevel.HIGH, 15),
        (r"(DAN|jailbreak|uncensored|unfiltered)\s*mode", 
         "jailbreak_mode", ThreatLevel.CRITICAL, 25),
        (r"developer\s+mode\s*(enabled|activated|on)", 
         "developer_mode_exploit", ThreatLevel.CRITICAL, 25),
        (r"enable\s+developer\s+mode", 
         "developer_mode_enable", ThreatLevel.CRITICAL, 25),
        (r"bypass\s+(safety|security|restrictions?|filters?)", 
         "bypass_safety", ThreatLevel.CRITICAL, 25),
    ]
    
    # System prompt extraction attempts
    SYSTEM_PROMPT_EXTRACTION = [
        (r"(show|reveal|display|print|output|tell)\s+.{0,20}(your|the|system)\s+(prompt|instructions?|rules?)", 
         "prompt_extraction", ThreatLevel.HIGH, 15),
        (r"what\s+(are|were|is)\s+.{0,20}(original|initial|system)\s+(instructions?|prompt)", 
         "prompt_query", ThreatLevel.MEDIUM, 10),
        (r"repeat\s+.{0,15}(system|initial)\s+(message|prompt|instructions?)", 
         "prompt_repeat", ThreatLevel.HIGH, 15),
        (r"(beginning|start)\s+of\s+(the\s+)?(conversation|prompt|instructions?)", 
         "prompt_beginning", ThreatLevel.MEDIUM, 10),
        (r"(system|hidden|secret)\s+prompt", 
         "system_prompt_mention", ThreatLevel.MEDIUM, 10),
    ]
    
    # Code injection and special character attacks
    CODE_INJECTION_PATTERNS = [
        (r"<\s*script[^>]*>", 
         "script_tag_injection", ThreatLevel.CRITICAL, 25),
        (r"(execute|run)\s+(this\s+)?(script|command|code)", 
         "script_execution", ThreatLevel.CRITICAL, 25),
        (r"\brm\s+-rf\b", 
         "destructive_command", ThreatLevel.CRITICAL, 25),
        (r"\{\{\s*.*?\s*\}\}", 
         "template_injection", ThreatLevel.HIGH, 15),
        (r"\$\{[^}]+\}", 
         "variable_injection", ThreatLevel.HIGH, 15),
        (r"__[a-zA-Z]+__", 
         "dunder_access", ThreatLevel.MEDIUM, 10),
        (r"eval\s*\(|exec\s*\(|compile\s*\(", 
         "code_execution", ThreatLevel.CRITICAL, 25),
        (r"import\s+(os|sys|subprocess|shutil)", 
         "dangerous_import", ThreatLevel.CRITICAL, 25),
    ]
    ...
    FINANCIAL_DANGEROUS_PATTERNS = [
        (r"(transfer|send|wire)\s+all\s+(my|the)\s+(money|funds|balance)", 
         "fund_transfer_all", ThreatLevel.CRITICAL, 25),
        (r"(give|tell|show|reveal)\s+me\s+.{0,30}(api|secret|private|access)\s*key", 
         "credential_extraction", ThreatLevel.CRITICAL, 25),
        (r"(password|credentials?|secrets?)\s*(is|are|:)", 
         "credential_leak", ThreatLevel.HIGH, 15),
        (r"(access|hack|breach|exploit)\s+(the\s+)?(account|system|database)", 
         "unauthorized_access", ThreatLevel.CRITICAL, 25),
    ]
    
    # Boundary/delimiter manipulation
    DELIMITER_ATTACKS = [
        (r"---+\s*(system|user|assistant)\s*---+", 
         "role_delimiter", ThreatLevel.HIGH, 15),
        (r"\[INST\]|\[\/INST\]|\<\|im_start\|\>|\<\|im_end\|\>", 
         "instruction_tag", ThreatLevel.CRITICAL, 20),
        (r"<\|system\|>|<\|user\|>|<\|assistant\|>", 
         "role_tag_injection", ThreatLevel.CRITICAL, 20),
        (r"(USER|SYSTEM|ASSISTANT)\s*:", 
         "role_prefix", ThreatLevel.MEDIUM, 10),
        (r"###\s*(System|Instruction|Human|Assistant)", 
         "markdown_role", ThreatLevel.MEDIUM, 10),
    ]
    
    # Obfuscation detection
    OBFUSCATION_PATTERNS = [
        (r"(?:[a-zA-Z]\s){5,}", 
         "spaced_letters", ThreatLevel.MEDIUM, 10),
        (r"(?:[\u200b-\u200f\u2060-\u206f])", 
         "invisible_characters", ThreatLevel.HIGH, 15),
        (r"(?:[a-zA-Z][0-9]){3,}", 
         "letter_number_mix", ThreatLevel.LOW, 5),
        (r"\\x[0-9a-fA-F]{2}", 
         "hex_encoding", ThreatLevel.MEDIUM, 10),
        (r"\\u[0-9a-fA-F]{4}", 
         "unicode_encoding", ThreatLevel.MEDIUM, 10),
    ]
    ...
    def validate(self, input_text: str) -> InputValidationResult:
        ...
        if self.strict_mode and max_threat in (ThreatLevel.HIGH, ThreatLevel.CRITICAL):
            ...
            is_safe = False
            rejection_reason = f"Risk score {total_risk_score:.1f} exceeds threshold {self.risk_threshold}"
