...
class OutputGuard:
    ...
    PROFANITY_PATTERNS = [
        ...
        (
            ...
            "insult",
            ContentIssueType.TOXIC_CONTENT,
            Severity.HIGH,
        ),
    ]
    ...
    HARMFUL_ADVICE_PATTERNS = [
        ...
        (
            ...
            "guarantee_returns",
            ContentIssueType.HARMFUL_ADVICE,
            Severity.CRITICAL,
    ...
    HALLUCINATION_PATTERNS = [
        ...
        (
            ...
            ContentIssueType.HALLUCINATION,
            Severity.MEDIUM,
    ...
    def __init__(
        ...
        self._disclaimer_patterns = [re.compile(p, re.IGNORECASE) for p in self.DISCLAIMER_TRIGGERS]

    def _compile_pii_patterns(
        self,
    ) -> List[Tuple[re.Pattern, str, ContentIssueType, Severity, str]]:
        """Compile PII patterns with replacement strings."""
        compiled = []
        for pattern, name, issue_type, severity, replacement in self.PII_PATTERNS:
            try:
                compiled.append(
                    (re.compile(pattern, re.IGNORECASE), name, issue_type, severity, replacement)
                )
    ...
    def _compile_basic_patterns(
        ...
        for item in patterns:
            ...
            try:
                compiled.append(
                    (re.compile(pattern, re.IGNORECASE), name, issue_type, severity, description)
                )
            except re.error as e:
                logger.error(f"Failed to compile pattern '{name}': {e}")
        return compiled

    def _compile_harmful_patterns(
        self,
    ) -> List[Tuple[re.Pattern, str, ContentIssueType, Severity, str]]:
...
def get_safe_response(
    content: str, fallback: str = "I cannot provide that response.", **kwargs
) -> str:
    """
