...
async def analyze_finances(
    ...
    try:
        ...
        safe_response = validate_ai_output(
            result.response,
            context={"user_provided_data": str(analysis_request.context) if analysis_request.context else ""},
        )
...
class CategoryCorrectionRequest(BaseModel):
    ...
    merchant_raw: str = Field(..., description="Raw merchant description", min_length=1, max_length=500)
    ...
    merchant_normalized: Optional[str] = Field(None, description="Normalized merchant name if available")
...
async def submit_category_correction(
    ...
    try:
        ...
        if result.get("auto_updated"):
            ...
            message = f"Correction recorded. Consensus reached for '{result['consensus_category']}'."
