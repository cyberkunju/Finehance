...
class FileImportService:
    ...
    def _parse_amount(self, amount_value: any, row_number: int) -> Decimal:
        ...
        try:
            ...
            raise ValueError(
                f"Invalid amount '{amount_value}' at row {row_number}: {str(e)}"
            )
        ...
        for parsed_tx in parsed_transactions:
            try:
                ...
                transaction_data = TransactionCreate(
                    ...
                    date=parsed_tx.date.date(),
                    description=parsed_tx.description,
                    category=parsed_tx.category or ("Uncategorized" if not auto_categorize else None),
                    type=TransactionType(parsed_tx.type) if parsed_tx.type else TransactionType.EXPENSE,
