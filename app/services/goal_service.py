...
class GoalService:
        ...
        if goal.deadline and goal.current_amount > 0:
            ...
            if days_since_creation > 0:
                ...
                if daily_rate > 0:
                    ...
                    estimated_completion_date = datetime.utcnow().date() + timedelta(days=days_to_completion)
                    
                    # Check if at risk
                    if estimated_completion_date > goal.deadline:
                        is_at_risk = True
                        days_behind = (estimated_completion_date - goal.deadline).days
                        risk_reason = f"Projected to miss deadline by {days_behind} days at current rate"
    ...
    async def update_goal_progress_from_transaction(
        ...
        updated_goals = []
        for goal in goals:
            updated_goal = await self.update_goal_progress(
                goal.id,
                user_id,
                transaction_amount
            )
        ...
        for goal in goals:
            ...
            if progress and progress.is_at_risk:
                ...
                if progress.days_remaining and progress.days_remaining < 30:
                    ...
                    severity = "WARNING"
                    recommended_action = "Consider increasing your contribution rate to stay on track"
                
                alerts.append(GoalRiskAlert(
                    goal_id=goal.id,
                    name=goal.name,
                    severity=severity,
