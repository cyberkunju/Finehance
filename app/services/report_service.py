...
class ReportService:
    ...
    async def generate_report(
        ...
        budget_adherence = await self._analyze_budget_adherence(
        ...
        if compare_to_previous_period:
            spending_changes = await self._detect_spending_changes(
                user_id, start_date, end_date
            )
    ...
    async def _detect_spending_changes(
        ...
        current_spending = await self._get_spending_by_category(
            user_id, start_date, end_date
        )
    ...
    def export_to_csv(self, report: FinancialReport) -> bytes:
        ...
        writer.writerow(["Average Transaction", f"${report.expense_summary.average_transaction:.2f}"])
        writer.writerow([])
        ...
        if report.budget_adherence:
            ...
            writer.writerow(["Overall Adherence", f"{report.budget_adherence.overall_adherence:.1f}%"])
            writer.writerow([])
        ...
        if report.spending_changes:
            ...
            writer.writerow(["Category", "Previous", "Current", "Change", "Change %", "Significant"])
    ...
    def export_to_pdf(self, report: FinancialReport) -> bytes:
        ...
        try:
            ...
            # Report period
            story.append(Paragraph(f"<b>Report Period:</b> {report.start_date} to {report.end_date}", styles['Normal']))
            ...
            if report.expense_summary.expenses_by_category:
                ...
                expense_category_table = Table(expense_category_data, colWidths=[2.5*inch, 1.5*inch, 1*inch])
                expense_category_table.setStyle(TableStyle([
            ...
            savings_color = colors.HexColor('#27ae60') if report.net_savings >= 0 else colors.HexColor('#e74c3c')
            savings_data = [
            ...
            if report.budget_adherence:
                ...
                story.append(Paragraph(f"Budget: {report.budget_adherence.budget_name}", styles['Normal']))
                story.append(Paragraph(f"Overall Adherence: {report.budget_adherence.overall_adherence:.1f}%", styles['Normal']))
            ...
            if report.spending_changes:
                ...
                changes_table = Table(changes_data, colWidths=[1.5*inch, 1.2*inch, 1.2*inch, 1.2*inch, 1*inch])
