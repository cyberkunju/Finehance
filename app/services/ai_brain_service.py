...
class CircuitBreaker:
    ...
    async def __call__(self):
        ...
        if not await self.can_execute():
            raise CircuitBreakerOpenError(f"Circuit {self.name} is OPEN, request rejected")
...
class AIBrainService:
    ...
    def __init__(
        ...
        self.model_path = model_path or getattr(
            settings, "ai_brain_model_path", "./ai_brain/models/financial-brain-qlora"
        )
        self._brain = None
        self._http_client = None
        self._available = None
        self._last_check = 0.0  # Timestamp of last availability check
    ...
    async def query(
        ...
        try:
            if self.mode == "http":
                result = await self._query_http_with_resilience(
                    query, mode, context, conversation_history
                )
            else:
    ...
    def _parse_transaction_fallback(self, description: str) -> Dict:
            ...
            try:
                ...
                if merchant_info:
                    ...
                    return {
                        ...
                        if "online" in (merchant_info.subcategory or "").lower()
                        else "retail",
                        "is_recurring": merchant_info.is_recurring,
                        "confidence": min(
                            0.9, merchant_info.match_score + 0.2
                        ),  # High confidence from DB
        ...
        return {
            ...
            "merchant_type": "online"
            if any(kw in description_upper for kw in ["ONLINE", "WWW", ".COM", "AMZN"])
            else "retail",
    ...
    def _generate_analysis_fallback(self, context: Optional[Dict]) -> str:
        ...
        if "spending" in context:
            ...
            top_categories = sorted(context["spending"].items(), key=lambda x: x[1], reverse=True)[
                :3
            ]
            parts.append(f"Your total monthly spending is ${total_spending:,.2f}.")
            parts.append(
                f"Top spending categories: {', '.join(f'{cat} (${amt:,.2f})' for cat, amt in top_categories)}."
            )
    ...
    async def parse_transaction(
        ...
        if RAG_AVAILABLE:
            try:
                ...
                enriched_context = {
                    "rag_context": rag_context_str,
                    "merchant_hint": rag_context.merchant_info.canonical_name
                    if rag_context.merchant_info
                    else None,
