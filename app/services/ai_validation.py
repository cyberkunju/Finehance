...
class AIMLCrossValidator:
    ...
    AI_PREFERRED_CATEGORIES = {"Transfers", "Income", "Fees", "Other"}
    ...
    def _decide_category(
        ...
        if ml_confidence > ai_confidence and ml_confidence >= 0.8:
            return (
                ml_category,
                True,
                f"ML higher confidence ({ml_confidence:.2f} > {ai_confidence:.2f})",
            )
...
class FinancialRulesEngine:
    ...
    def validate_advice(
        ...
        if re.search(
            r"get\s+rich\s+quick|easy\s+money|guaranteed\s+returns", response, re.IGNORECASE
        ):
            issues.append(
                {
